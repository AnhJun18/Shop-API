...
public class PromotionController {
    ...
    public ApiResponse<?> removeProductToPromotion(@RequestBody AddPrToPromotionRequest addPrToPromotionRequest) {

        try {
            promotionService.removeProductToPromotion(addPrToPromotionRequest);
            return ApiResponse.builder().status(HttpStatus.SC_OK).message("Xóa khuyến mãi thành công").build();
        } catch (Exception e) {
            return ApiResponse.builder().status(HttpStatus.SC_INTERNAL_SERVER_ERROR).message(e.toString()).build();
        }
    }
