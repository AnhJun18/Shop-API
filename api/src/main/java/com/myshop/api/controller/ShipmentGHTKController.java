...
public class ShipmentGHTKController {
    ...
    public Map<String, Object> createOrder(@PathVariable(name = "id") Long order_id)
        ...
        order.put("name",  od.getUserInfo().getLastName());
        order.put("address", od.getAddress());
        order.put("province", od.getProvince());
        order.put("district", od.getDistrict());
        order.put("ward", od.getWard());
        order.put("hamlet", "Khác");
        order.put("tel", od.getUserInfo().getPhone());
